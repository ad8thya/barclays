...
if score_res.get("data"):
    ...
    # Verify graph data has multi-type nodes
    graph = d.get("graph", {})
    if graph.get("campaign_id"):
        print(f"       Campaign ID: {graph['campaign_id']}")
...
if graph_res.get("data"):
    ...
    print(f"       Nodes: {len(nodes)} ({types})")
    print(f"       Edges: {len(edges)} ({rels})")
    # Verify no node is missing type
    missing_type = [n["id"] for n in nodes if not n.get("type")]
    if missing_type:
        print(f"  [WARN] Nodes missing type: {missing_type}")
    # Verify campaign_id populated
    with_camp = [n["id"] for n in nodes if n.get("campaign_id")]
    print(f"       Nodes with campaign_id: {len(with_camp)}")
